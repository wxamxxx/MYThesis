...
In this analysis, prompt isolated electrons
...
in Chapter~\ref{chap:reco}. Selected jets are required to satisfy
...
Table~\ref{tab:object:jet} and Table~\ref{tab:object:rcjet}.
...
Chapter~\ref{chap:reco}. The algorithm exploits lifetime-sensitive
...
\begin{table}[ht]
  \caption{\bTagHead{} definition used in the analysis.}
...
in the event reconstruction, kinematic observables and corresponding systematics estimation.
...
multiplicity characteristic of $t\bar{t}t\bar{t}$ productions
...
%---------------------------------------------------------
\subsubsection{Baseline jet and \bJetHead{} requirements}
%---------------------------------------------------------

To enhance sensitivity to multi-top productions,
...
Section~\ref{subsec:background_modelling}, a dedicated correction procedure is applied
to improve the modelling of the $t\bar{t}$+jets background.
The corrections are derived in regions where the potential
signal contamination is verified to be negligible: regions with
relatively low jet multiplicity are
used to determine the flavour-dependent normalisation rescaling,
while regions with low $b$-jet multiplicity are used to derive the
kinematic reweighting.

Events in regions with at least three $b$-tagged jets are used in the
statistical analysis. A multivariate discriminant is developed,
as described in Section~\ref{subsec:multivariate_strategy}, and is used
throughout the analysis. The MVA output distribution, together with the
$H_\mathrm{T}$ distribution in selected regions, is included in a
