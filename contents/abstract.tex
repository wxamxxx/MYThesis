...
\begin{abstract}[zh]
  ...
  且进行了一系列方法鲁棒性检验。此分析为未来四顶夸克微分测量提供了系统的方法学基础。

  此外，本文还开展了与ATLAS阻性板室（RPC）探测器相关的研究工作。包括基于探测器控制系统数据开发自动化监测工具，实现了Run~3取数期间探测器性能的近实时监控与异常行为识别。
...
\begin{abstract}[en]
  ...
  To further exploit the physics potential of $t\bar{t}t\bar{t}$ production, a reconstruction and unfolding framework is developed for differential measurements in partial Run~3 data.
  A dedicated reconstruction strategy based on a blended graph–hypergraph neural network framework is introduced to assign reconstructed objects to the four top-quark decay chains, addressing the severe combinatorial complexity of the final state.
  The kinematics of multiple neutrinos are inferred using analytic technique or machine-learning-based approach, enabling the reconstruction of parton-level observables such as the $t\bar{t}t\bar{t}$ invariant mass $m_{4t}$ and other high-energy variables sensitive to new physics effects.
  Despite the intrinsic ambiguity of the topology, the method achieves matching efficiencies of about 60\% for fully reconstructed top quarks, with a truth–reconstruction correlation of approximately 0.6 for $m_{4t}$ in the same-sign dilepton topology, demonstrating a significant improvement over random assignment.
