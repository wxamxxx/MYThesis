...
\begin{abstract}[zh]
  ...
  粒子物理致力于研究组成宇宙的基本粒子及其相互作用，其中标准模型是目前最为成功的理论框架，将所有已知的基本粒子和三种基本相互作用统一描述，并成功地由大量实验验证。
  ...
  四顶夸克产生过程是标准模型中极为罕见的反应之一，并于2023年首次被ATLAS实验观测，其测量结果在不确定度范围内与标准模型预言一致，但偏高的中心值仍为新物理效应留下了潜在空间。

  本论文基于大型强子对撞机ATLAS实验在Run~2中以$\sqrt{s}=13~\mathrm{TeV}$条件下收集的139~fb$^{-1}$数据，在单轻子及双反电荷轻子末态中开展四顶夸克事例的研究，
  ...
  在$H$与$A$共同对反应截面贡献的假设下，对应排除$\tan\beta < 1.7$和$0.7$的参数空间。同时，在$S_8S_8 \to t\bar{t}t\bar{t}$的搜索中排除了质量低于1.3~TeV的$S_8$。
  ...
  在此基础上，采用基于剖面似然函数的展开方法提取微分截面，并系统研究了正则化策略及展开方法稳健性。该框架为未来四顶夸克微分测量提供了系统的方法论基础。

  此外，本文还开展了与ATLAS阻性板室（RPC）探测器相关的研究工作。包括基于探测器控制系统数据开发自动化性能监控工具，以及参与Phase-II升级相关的读出板设计与质量控制测试研究。
  同时，本文讨论了通过有限元方法对气体流动与电场分布进行模拟，评估结构参数对探测器性能的影响，为未来优化设计提供参考。
...
\begin{abstract}[en]
  ...
  In the scenario where both $H$ and $A$ contribute to the cross section, values of $\tan\beta$ below 1.7 and 0.7 are excluded for masses of 400~GeV and 1000~GeV, respectively. 
  ...
  The kinematics of multiple neutrinos are inferred using both analytic techniques and machine-learning-based approaches, enabling the reconstruction of parton-level observables such as the $t\bar{t}t\bar{t}$ invariant mass $m_{4t}$ and other high-energy variables sensitive to new physics effects.
  ...
  An optimised spacer configuration is shown to reduce the number of spacers by about 24\%, thereby reducing inactive regions while preserving detector performance, achieving improved gas-flow uniformity and maintaining detection efficiencies above 95\%.
